  ...
  return (
    <div className="glass rounded-xl p-6 flex flex-col gap-6">
      ...
      <div className="border-t border-slate-700 pt-4">
        ...
        {emergencyActive && emergencyLane && (
          <div className="mb-3 p-3 bg-red-950/60 border border-red-500/40 rounded-xl text-sm animate-pulse">
            <p className="text-red-400 font-bold">
              🚨 Emergency active — {emergencyLane.toUpperCase()} lane
            </p>
            <p className="text-red-500/70 text-xs mt-0.5">
              Normal cycles are frozen. Auto-clears in {emergencySeconds}s.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-2">
          {LANES.map(lane => (
            <EmergencyToggle
              key={lane}
              lane={lane}
              disabled={emergencyActive && emergencyLane !== lane}
            />
          ))}
        </div>
      </div>
